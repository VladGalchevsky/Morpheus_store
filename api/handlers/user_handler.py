...
async def _update_user(updated_user_params: dict, user_id: UUID, user_dal: UserDAL) -> UUID | None:
        updated_user_id = await user_dal.update_user(user_id=user_id, **updated_user_params)
        return updated_user_id

async def _get_user_by_id(user_id: UUID, user_dal: UserDAL) -> UserWithOrderSummary | None:
